...
def check_wix_install(wix_install_path=WIX_INSTALL_PATH,
    ...
    if not m:
        raise DistutilsError("Can't parse version output from candle.exe: {}"
                             "".format(verline))
    wix_version = [int(m.group("major")), int(m.group("minor"))]
    wix_min_version = [int(i) for i in wix_required_version.split(".")[:2]]
    if (wix_version[0] < wix_min_version[0]
        or (wix_version[0] == wix_min_version[0]
            and wix_version[1] < wix_min_version[1])):
        raise DistutilsError("Minimal WiX v{}, we found v{}"
                             "".format(".".join(wix_min_version),
                                       ".".join(wix_version)))
