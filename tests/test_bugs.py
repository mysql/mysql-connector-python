...
class Bug380528(tests.MySQLConnectorTests):
    ...
    def test_old_password(self):
        ...
        try:
            cur.execute(
                "GRANT SELECT ON %s.* TO %s" % (self.config["database"], user)
            )
        ...
        self.assertRaises(
            errors.NotSupportedError, self.cnx.__class__, **test_config
        )
...
class Bug507466(tests.MySQLConnectorTests):
    ...
    def test_bits(self):
        ...
        select = (
            "SELECT c1,c2,c3,c4,c5,c6,c7,c8 FROM myconnpy_bits ORDER BY id"
        )
...
class Bug571201(tests.MySQLConnectorTests):
    ...
    def test_multistmts(self):
        ...
        cur.execute(
            (
                "CREATE TABLE {0} ( " "id INT AUTO_INCREMENT KEY, " "c1 INT)"
            ).format(self.tbl)
        )
...
class Bug809033(tests.MySQLConnectorTests):
    ...
    def test_lost_connection(self):
        ...
        def sleepy_select(cnx):
            ...
            # Ugly query ahead!
            stmt = (
                "SELECT x1.*, x2.* from {table} as x1, {table} as x2".format(
...
class BugOra13395083(tests.MySQLConnectorTests):
    ...
    def test_time_zone(self):
        ...
        row = cur.fetchone()
        self.assertEqual(
            now_utc.astimezone(testzone), row[0].replace(tzinfo=testzone)
        )
...
class BugOra13392739(tests.MySQLConnectorTests):
    ...
    def test_ping(self):
        ...
        try:
            ...
            self.fail(
                "Error raised although connection should be "
                "available (%s)." % e
            )
    ...
    def test_reconnect(self):
        ...
        try:
            ...
            self.fail(
                "Errors raised although connection should have been "
                "reconnected."
            )
...
@unittest.skipIf(
    tests.MYSQL_VERSION > (8, 0, 4), "Revoked users can no more grant"
)
...
class BugOra16662920(tests.MySQLConnectorTests):
    ...
    def setUp(self):
        ...
        data = [("a",), ("c",), ("e",), ("d",), ("g",), ("f",)]
        cur.executemany(
            "INSERT INTO {0} (c1) VALUES (%s)".format(self.tbl), data
        )
        cur.close()
        cnx.commit()
...
class BugOra17041412(tests.MySQLConnectorTests):
    ...
    def _setup(self):
        ...
        cur.executemany(
            "INSERT INTO %s (c1) VALUES (%%s)" % self.table_name, self.data
        )
        cnx.commit()
...
class BugOra17041240(tests.MySQLConnectorTests):
    ...
    def _setup(self):
        ...
        cur.execute(
            "CREATE TABLE {table} (c1 INT)".format(table=self.table_name)
        )
        cur.executemany(
    ...
    def test_cursor_close(self):
        ...
        cur = self.cnx.cursor()
        cur.execute(
            "SELECT * FROM {table} ORDER BY c1".format(table=self.table_name)
        )
        self.assertEqual(self.data[0], cur.fetchone())
...
class BugOra17573172(tests.MySQLConnectorTests):
    ...
    def test_read_only(self):
        if self.cnx.get_server_version() < (5, 6, 5):
            self.assertRaises(
                ValueError, self.cnx.start_transaction, readonly=True
            )
        else:
            self.cnx.start_transaction(readonly=True)
            self.assertTrue(self.cnx.in_transaction)
            self.assertRaises(
                errors.ProgrammingError, self.cnx.start_transaction
            )
...
class BugOra17965619(tests.MySQLConnectorTests):
    ...
    def setUp(self):
        ...
        procedure = (
            "CREATE PROCEDURE `proce_with_binary` "
...
class BugOra16217765(tests.MySQLConnectorTests):
    ...
    def _create_user(self, cnx, user, password, host, database, plugin):
        ...
        create_user = (
            "CREATE USER '{user}'@'{host}' " "IDENTIFIED WITH {plugin}"
        )
    ...
    def test_sha256_nonssl(self):
        ...
        config["auth_plugin"] = user["auth_plugin"]
        self.assertRaises(
            errors.InterfaceError, connection.MySQLConnection, **config
        )
        if CMySQLConnection:
            self.assertRaises(
                errors.InterfaceError, CMySQLConnection, **config
            )
...
class BugOra18389196(tests.MySQLConnectorTests):
    ...
    def test_parameters(self):
        ...
        try:
            ...
            self.fail(
                "Inserting parameter multiple times in a statement "
                "failed: %s" % err
            )
...
class BugOra18415927(tests.MySQLConnectorTests):
    ...
    def setUp(self):
        ...
        cnx.cmd_query(
            create_user.format(user=self.user["username"], host=host)
        )
        ...
        cnx.cmd_query(
            grant.format(
                database=database, user=self.user["username"], host=host
            )
        )
...
class BugOra18220593(tests.MySQLConnectorTests):
    ...
    def test_unicode_operation(self):
        ...
        self.cur.executemany(
            "INSERT INTO {0} VALUES (%s)".format(self.table), data
        )
        self.cnx.commit()
    ...
    def test_compress(self):
        for charset in ("utf8", "latin1", "latin7"):
            ...
            try:
                ...
                traceback.print_exc()
                self.fail(
                    "Failed setting up compressed cnx using {0}".format(
                        charset
                    )
                )
            except errors.Error:
...
class BugOra19169143(tests.MySQLConnectorTests):
    ...
    def test_duplicate_optionfiles(self):
        ...
        self.assertRaises(
            ValueError, mysql.connector.connect, option_files=files
        )
...
class BugOra21530100(tests.MySQLConnectorTests):
    ...
    def test_option_files_with_option_groups(self):
        ...
        try:
            ...
            self.fail(
                "Connection failed with option_files argument: {0}"
                "".format(exc)
            )
        finally:
...
class BugOra19522948(tests.MySQLConnectorTests):
    ...
    def test_row_to_python(self):
        ...
        data = "test_data" * 10
        cur.execute(
            "INSERT INTO {0} (c1) VALUES (?)".format(self.tbl), (data,)
        )
    ...
    def test_password_with_spaces(self):
        ...
        for user, password in self._credentials:
            ...
            try:
                ...
                self.fail(
                    "Failed using password with spaces for user %s" % user
                )
            else:
                cnx.close()
...
class BugOra21476495(tests.MySQLConnectorTests):
    ...
    def test_bad_set_charset_number(self):
        ...
        self.assertRaises(
            mysql.connector.Error, self.cnx.set_charset_collation, 19999
        )
...
class BugOra21477493(tests.MySQLConnectorTests):
    ...
    def test_insert_into_select_type1(self):
        ...
        cursor = self.cnx.cursor()
        cursor.executemany(
            "INSERT INTO fun1 SELECT CONCAT('VALUES', %s), "
            "b + %s FROM fun1",
            data,
        )
        cursor.close()
    ...
    def test_insert_into_select_type2(self):
        ...
        cursor = self.cnx.cursor()
        cursor.executemany(
            "INSERT INTO fun1 SELECT CONCAT('VALUES(ab, cd)',"
            "%s), b + %s FROM fun1",
            data,
        )
        cursor.close()
...
class BugOra21492815(tests.MySQLConnectorTests):
    ...
    def setUp(self):
        ...
        create = "CREATE PROCEDURE {0}() BEGIN SELECT 1234; " "END".format(
            self.proc1
        )
        cur.execute(create)
    ...
    def test_set(self):
        ...
        self.assertEqual(
            (bytearray(b"1234"),), next(cur.stored_results()).fetchone()
        )
...
class BugOra21656282(tests.MySQLConnectorTests):
    ...
    def setUp(self):
        ...
        self.cnx.cmd_query(
            grant.format(
                database=config["database"], user=self.user, host=self.host
            )
        )
...
@unittest.skipIf(
    sys.version_info < (2, 7, 9), "Python 2.7.9+ is required for SSL"
)
class BugOra25397650(tests.MySQLConnectorTests):
    ...
    def _verify_cert(self, config):
        ...
        config["ssl_verify_cert"] = True
        self.assertRaises(
            errors.InterfaceError, mysql.connector.connect, **config
        )
        config["ssl_verify_cert"] = False
...
class BugOra25589496(tests.MySQLConnectorTests):
    ...
    def test_insert_binary(self):
        ...
        new_row = cursor.execute(
            add_row_q, {"section": "foo", "pickled": pickled}
        )
        self.cnx.commit()
...
class BugOra22564149(tests.MySQLConnectorTests):
    ...
    def test_cmd_query_iter(self):
        stmt = (
            "SELECT 1; INSERT INTO {0} VALUES (1, 'João'),(2, 'André'); "
            "SELECT 3"
        )
        results = []
...
class Bug26484601(tests.MySQLConnectorTests):
    ...
    def try_connect(self, tls_version, expected_ssl_version):
        ...
        if isinstance(expected_ssl_version, tuple):
            ...
            server_tls = tuple(
                [int(d) for d in (res[0][1].split("v")[1].split("."))]
            )
    ...
    def test_cursor_dict(self):
        ...
        while row is not None:
            ...
            self.assertEqual(
                exp[i],
                row,
                "row {} is not equal to expected row"
                " {}".format(row, exp[i]),
            )
            row = cur.fetchone()
            i += 1
...
class Bug27897881(tests.MySQLConnectorTests):
    ...
    def test_retrieve_from_LONGBLOB(self):
        ...
        for num, expected_value in zip(
            range(len(test_values)), expected_values
        ):
...
class Bug27489937(tests.MySQLConnectorTests):
    ...
    def test_cext_pool_support(self):
        ...
        self.assertRaises(
            errors.PoolError, mysql.connector.connect, **self.config
        )
...
class BugOra30416704(tests.MySQLConnectorTests):
    ...
    def test_binary_charset(self):
        self.cnx.set_charset_collation("binary")
        self._test_result(
            self.cnx, self.exp_binary_values, use_binary_charset=True
        )
    ...
    def test_parameters_type(self):
        ...
        with self.cnx.cursor(prepared=True) as cur:
            ...
            for param in ["12", 12, 1.3, lambda: "1" + "2"]:
                self.assertRaises(
                    errors.ProgrammingError, cur.execute, stmt, (param,)
                )
                with self.assertRaises(errors.ProgrammingError) as contex:
        ...
        with self.cnx.cursor(prepared=True) as cur:
            ...
            self.assertRaises(
                errors.ProgrammingError, cur.execute, select_stmt, ("JD")
            )
            with self.assertRaises(errors.ProgrammingError) as contex:
...
class Bug32162928(tests.MySQLConnectorTests):
    ...
    def setUp(self):
        ...
        for new_user in self.new_users:
            cnx.cmd_query(
                "DROP USER IF EXISTS '{user}'@'{host}'".format(**new_user)
            )
    ...
    def test_change_user(self):
        ...
        for user1, user2 in test_cases:
            ...
            self.assertEqual(
                "{user}@{host}".format(**self.users[user1]), first_user
            )
            ...
            self.assertEqual(
                "{user}@{host}".format(**self.users[user2]), current_user
            )
            cnx_test.close()
    ...
    def test_prepared_statement_bigints(self):
        ...
        for prepared in prepared_options:
            with self.cnx.cursor(prepared=prepared) as cur:
                ...
                for tc in test_cases:
                    query = (
                        f"INSERT INTO {self.table_name} (bigints) VALUES (%s)"
                    )
                    cur.execute(query, (tc,))
                    self.cnx.commit()
