...
class MySQLConnectionTests(tests.MySQLConnectorTests):
    ...
    def test_allow_local_infile_in_path(self):
        ...
        def create_table():
            def_cur.execute("DROP TABLE IF EXISTS {}.local_data_in_path"
                            "".format(database))
            def_cur.execute("CREATE TABLE {}.local_data_in_path "
                            "(id int, c1 VARCHAR(6), c2 VARCHAR(6))"
                            "".format(database))

        def verify_load_success(cur, data_file, exp):
            sql = ("LOAD DATA LOCAL INFILE %s INTO TABLE {}.local_data_in_path"
                   "".format(database))
            cur.execute(sql, (data_file, ))
            cur.execute("SELECT * FROM {}.local_data_in_path"
                        "".format(database))

            self.assertEqual(exp, cur.fetchall())
            cur.execute("TRUNCATE TABLE {}.local_data_in_path"
                        "".format(database))
            cur.close()

        def verify_load_fails(cur, data_file, err_msgs,
            ...
            if isinstance(err_msgs, (list, tuple)):
                ...
                self.assertTrue(
                    (err_msgs in exception_msg), "Unexpected exception "
                    "message found: {}".format(context.exception))
            cur.close()

        exp1 = [(1, "c1_1", "c2_1"), (2, "c1_2", "c2_2"),
                (3, "c1_3", "c2_3"), (4, "c1_4", "c2_4"),
                (5, "c1_5", "c2_5"), (6, "c1_6", "c2_6")]

        exp2 = [(10, "c1_10", "c2_10"), (20, "c1_20", "c2_20"),
                (30, "c1_30", "c2_30"), (40, "c1_40", "c2_40"),
                (50, "c1_50", "c2_50"), (60, "c1_60", "c2_60")]
        ...
        cur = cnx.cursor()
        data_file = os.path.join("tests", "data", "local_data.csv")
        verify_load_fails(cur, data_file, ["INFILE file request rejected",
        ...
        cnx = connector_class(**settings)
        cur = cnx.cursor()
        data_file = os.path.join("tests", "data", "in_file_path",
                                 "local_data_in_path.csv")
        ...
        data_file = os.path.join("tests", "data", "in_file_path", "..",
                                 "local_data.csv")
        verify_load_fails(cur, data_file, ("file request rejected", "not found",
                                           "command is not allowed"))
        ...
        if  os.name != "nt" and connector_class != CMySQLConnection:
            ...
            cnx = connector_class(**settings)
            cur = cnx.cursor()
            verify_load_fails(cur, link, "link is not allowed",
